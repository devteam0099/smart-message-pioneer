...
const TicketManagement = () => {
  ...
  const breadcrumbItems = ["User", "Contact Support"];
  ...
  const handleDelete = async (rowId) => {
    let url = `${process.env.REACT_APP_BASE_URL}/doublefollowers/v1/ticket/bulk/delete`;
    try {
      ...
      if (result?.code === 200 || result?.code === 1000) {
        ...
        toast.error(result?.data || `Failed to delete ticket`);
        return false;
      }
    } catch (error) {
      toast.error(error?.message);
